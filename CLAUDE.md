...
- **`luto/solvers/`**: Optimization solver interface and input data preparation
  ...
  - `col_builder.py`: the COLUMN side — `get_cols(data, base_year) -> (cols, col_support)` builds the column space from the base-year state alone (transition bounds, feasibility, sources, masks): `cols`, every unknown as one row of the long table, and beside it a `ColSupport` — supporting data that maps the sparse column variables onto the dense input arrays, . The column table carries NO attrs (2026-09-23): every column is labelled with its `block` (a string — ag / nonag / am / ag2ag / ag2nonag / nonag2ag — so a block is the mask `cols['block'].values == name`) and its fields (`m, j, k, am_idx, j_idx, from_*, local_r, cell`) say where it reads its input, so a coefficient is `input[m, cell, j]` at the block's columns; the support holds the input masks `valid_ag_mrj`, `valid_nonag_rk` and `valid_ag2ag` / `valid_ag2nonag` / `valid_nonag2ag` (one per source: the shape of its solved deltas); `region2cell`, the region of every cell as data variables, no attrs (`state` / `nrm` / `ibra` / `water_region` names as object strings, None where a cell has none), so a region's cells are `region2cell[layer].values == region` and its columns are those in its cells, `(region2cell[layer].values == region)[cols['cell'].values]` (there is no per-column copy); and, beside them, `cell2col` (cell × col, 1 where a variable sits in the cell: every cell's variables, for the layer families) and the `ag_mrj2col` / `nonag_rk2col` position → column grids (in the inputs' layout, every ag / non-ag column written at its own position), −1 = no column, the join families look up
  ...
  - `row_builder.py`: the row side's ROW GENERATION, written as the mirror of `col_builder.py` — `get_rows(inputs, cols, col_support)` at the top, one function that lists every family's `(A, table)` pair in `parts` (a family that is off returns `None, None`), whose body IS the model's row order — first the wide rows (demand, GHG, the GBF families, ag-mgt adoption, regional adoption, water, renewable targets: few rows, each wide over the columns), then the structural rows (ag-mgt link, renewable ceiling, source cap, node balance: millions of rows of a few entries each), and everything it calls below it in that same order. The model is one matrix, rows × cols: every family that is a SUM over cells produces its ROWS the same way — `gather` (one coefficient per column: every column reads its block's input at its own fields — ag at `[m, cell, j]`, an option's am at `[m, cell, j_idx]`, an arc at its source's `[m, local_r, j]` / `[local_r, k]`) → pick the row's columns — a regional family (water, renewable) by the region of the column's cell, `(col_support.region2cell[layer].values == region)[cols['cell'].values]`, regional adoption by the cap's own cell set at the column's cell; a layer family by weighing (`weight_rows(W) @ (cell2col @ diags(c))`, `col_support.cell2col`, built in `get_cols` from the table's `cell` field: the family's weight rows over cells times the coefficient laid on the support, one float32 product per entry) → `contract` (ONE loop over the stacked rows: the `SOLVER_COEFF_MIN` drop, then for the policy families the geomean row rescale and the floor again). The JOIN families (ag-mgt link, adoption, demand groups, node balance) look their nodes up on the column-id grids (`col_support.ag_mrj2col` / `nonag_rk2col`, in the inputs' own layout, −1 = no column / no row — built in `get_cols` by writing every ag / non-ag column at its own (m, cell, j) / (cell, k)), source cap groups the arcs by their source's flat position in (lm, lu, cell) order, and the ag-mgt rows take the am columns off the table by their fields (`am_idx`, `j_idx`, `m`). The GBF families mask a regional target to its cells with `col_support.region2cell['nrm']` (GBF3 in `IBRA_REG` mode: `region2cell['ibra']`, loaded by `data.py` from `bio_GBF3_IBRA_Regions.nc` in that mode only); a target region the layer does not know raises. `simulation.solve_timeseries` frees `cell2col` / `ag_mrj2col` / `nonag_rk2col` right after `get_rows` (only `get_rows` reads them; GBs at RES1), so they are not held through the solve. `get_rows` lays the biodiversity contribution on the support ONCE (`bio_S`) for the five GBF families. Every family is a `get_*` written out in full — `get_ag_mgt_link`, `get_ag_mgt_adoption`, `get_renewable_ceiling`, `get_demand`, `get_ghg`, `get_GBF2`, `get_GBF3_NVIS`, `get_GBF4_SNES`, `get_GBF4_ECNES`, `get_GBF8`, `get_regional_adoption_ag` / `_nonag` / `_nonag_sum`, `get_water`, `get_renewable`, `get_source_cap_ag` / `_nonag`, `get_node_balance_ag` / `_nonag` (19 in all; there is no cell-usage family — every arc leaves one node of its cell and lands on another, so the cell's node-balance rows already sum to Σ X = Σ base, and a second statement of the cell total would be a redundant equality whose constant differs from that one by float noise) — each ONE plain section read top to bottom (its own loop, skip rules, names and `make_part` call), with no generic wrapper taking behaviour in as lambdas and no shared row constructor or row-rule helper to jump into. Each returns a PART — the pair `(A, table)` from `row_table.make_part`, one fixed row schema — and the last section of `get_rows` walks the list of `(A, table)` pairs (`parts`, in the model's row order), drops the `(None, None)`s, and stacks them: `sparse.vstack(blocks)` into the ONE matrix, `xr.concat([ROW_SCHEMA, *tables], 'row', fill_value=ROW_FILL)` into the ROW TABLE (see below; no stacking function — the concat fills a field a family does not carry with −1 / None, the empty `ROW_SCHEMA` template keeps every field and dtype); `get_rows` returns `(A, rows)`, nothing beside them. `get_obj(econ, cols, inputs)` is the objective coefficient of every column (million AUD, dropped and floored), a plain vector over the columns. `LutoSolver(cols, rows, A, obj, inputs)` is A x T and obj · x — the two tables, the matrix and the objective vector, and the inputs it reads the names that spell `m` / `am_idx` in a variable's name off (`RowInputs.landmans`, the keys of `RowInputs.agman2lu`); `solve()` returns the raw x; `post_solve.post_solve(x, cols, col_support, inputs)` turns it into the LUTO format
...
**Other solving engines (in progress: CPLEX, NVIDIA cuOpt)**: we are working on letting a run solve with engines other than GUROBI — IBM CPLEX (CPU; the `cplex` / `docplex` bindings are already in `requirements.yml`, a licensed IBM ILOG CPLEX Optimization Studio 22.2 installation supplies the native runtime) and NVIDIA cuOpt (open-source, GPU; its PDLP first-order LP method targets very large models such as full-resolution years, at a looser default precision than barrier). No model code reads either yet: `LutoSolver` is GUROBI-only, there is no engine setting, and every solve goes through `gurobipy`. Since the row / column tables are plain arrays (`cols`, `rows`, and `A` as one scipy CSR beside them), a second engine would plug in at `solvers/solver.py` alone — `row_builder` / `col_builder` / `post_solve` stay solver-agnostic. Keep them that way.
...
4. **Solver Input**: `solvers/col_builder.get_cols` → `(cols, col_support)` (the unknowns, and what the row side reads beside them); `solvers/row_inputs.get_row_inputs` → `inputs` (the coefficient streams and targets); `solvers/row_builder.get_rows(inputs, cols, col_support)` → `(A, rows)` (the constraints); `solvers/row_builder.get_obj(row_inputs.get_economics(...), cols, inputs)` → `obj` (the objective coefficient of every column, built last so a year the bound pass stops never loads the economy streams); the solver takes them as they are, `LutoSolver(cols, rows, A, obj, inputs)` — the step reads col / row / obj
   ...
   - **The row table** (built by `row_builder.get_rows`, whose body IS the model's row order; stored and queried by `row_table.py`): every constraint family is a generator `get_<family>(...) -> part | None` — it takes `inputs` / `cols` / `side` (the five GBF families take the biodiversity contribution laid on the support, `bio_S`, in place of `side`) — returning its rows as ONE part, `row_table.make_part(family, A, rhs, sense, names, scale, **labels)`: what the solver takes (its block of A, per-row `rhs`, `sense`, `name`), the `scale` the readers restore raw units with, and — as keyword arguments, only where a report needs them — the labels that say what each row is about (the part is the pair `(A, table)`), over a fixed row schema: int fields `cell, m, j, k, am_idx, from_m, from_j, from_k, local_r` (int32, −1 where the family has none), `family` (the family's name; a string like the labels — nothing on the table is a code, and it carries no index: a family's rows are the mask `rows['family'].values == name`, 0.07 s at RES5 and 1.7 s at RES1 per mask, some 70 masks a year across the readers) and label fields `region, GBF_target, GBF4_presence, demand_commodity, demand_bound` (object dtype, the labels as they are — one shared object per row, None where the family has none; `region` is the same concept across the GBF, regional-cap, water and renewable families (renewable's state is its `region`), so a reader fills one column for every family; a label fewer families carry is prefixed with what carries it — `GBF_target`, the vegetation group / species / community a GBF3 / GBF4 / GBF8 row targets). A family of one global row (GHG, GBF2) passes no labels. `get_rows` vstacks the blocks into `A` and `xr.concat`s the tables back to back into `rows` (the solver's `rows`) — `A` the ONE CSR (rows × cols, a scipy sparse matrix, kept OUT of the Dataset: xarray holds no scipy sparse variable and attrs are dropped by most operations) and `rows`, an `xr.Dataset` on `row` = Constr.index (a part is the same pair for one family, `make_part` returns `(A, table)`; row i of `A` is row i of the table; `get_rows` returns `(A, rows)` and `A` travels as its own argument: `get_row_bounds(A, rows, cols)`, `LutoSolver(cols, rows, A, obj, inputs)`, `post_solve(x, cols, col_support, inputs)` takes no row): and `active` (a dropped row is flagged off — the table never shrinks). `row_table.rows_where` queries it (a mask over the table). `LutoSolver.formulate` creates ONE `addMVar` over the column table and ONE `addMConstr` over the row table's ACTIVE rows (a row `row_bounds.drop_redundant_rows` flagged off before the build never reaches the solver; its handle is None), names both, keeps the Gurobi handles on the table (`constr`); `solvers/tools.record_shadow_prices` is ONE query on the table: the duals read once (one batched `getAttr('Pi')` over the priced rows in the model) and written onto it as `rows['pi']` (NaN where not read, 0 on a row dropped before the build), then every CSV column read off the table at the priced rows — `pi · 1e6 / scale`, `pi · 1e6 · rhs`, the label columns as they are, the per-family presentation in `tools.PRICED` {family: (constraint label, unit)} plus four plain overrides — no per-family reader functions. `remove_constraints_by_name` flags rows inactive and removes them from Gurobi; `restore_constraints_by_name` puts them back. `solve()` returns the raw x over the column table; `post_solve.post_solve(x, cols, col_support, inputs)` scatters it into the LUTO format (`SolverSolution`: the maps and the dvars, nothing read off the model); `simulation.store_solution(data, year, solution, obj_val, inputs)` then computes Production and GHG from the stored dvars — Production the way the base year's is (`data.get_actual_production_lyr`, dvar · lu2pr · q · pr2cm, `threshold=0`: every share counted, as the demand rows counted it), GHG as Σ ghg · share over ag / ag-mgt / non-ag plus the ag → ag transition emissions and the off-land constant from the step's `RowInputs`, in float64 (what the GHG row summed, to the contract's drop and float32; reported whether or not the GHG limit is on). The converters' default `threshold=0.01` (`tools.ag_mrj_to_xr` / `non_ag_rk_to_xr` / `am_mrj_to_xr`: a cell whose total share is ≤ 1 % is zeroed) is a map CLEAN-UP, kept for `write_quantity`'s spatial layers and per-region tables, which therefore sum to slightly LESS than the total — at RES50 by 0.1–0.8 % per commodity, almost all of it ag-mgt slivers (found 2026-09-22 when a target year's Production, until then the demand row's own `q_block @ x`, was first computed this way; the contract's `SOLVER_COEFF_MIN` drop played no part at RES50).
   - **The column space** (`luto/solvers/col_builder.py`): every unknown of the model as ONE row of a long table, `cols` — an `xr.Dataset` on `col` = Var.index, the rows in block order (`ag | nonag | am | ag2ag | ag2nonag | nonag2ag`), each labelled with its `block` (an object string, one shared str per column: a block's columns are the mask `cols['block'].values == name`) — with the fields of each column, −1 where a field does not apply: `m, j` (the ag (lm, lu) the column lands on: own for ag, host for am, the TO fields of ag2ag / nonag2ag), `k` (own for nonag, the TO field of ag2nonag), `slot`, `am_idx`, `j_idx` (the (am, lu) slot of an am column, its option as a position in `RowInputs.agman2lu`, its land use's position within the option), `from_m, from_j, from_k, local_r` (where an arc comes from), `cell` (every column), `lb` / `ub` (float64), `base` (float32, the node-balance constant) and nothing from the row side (the objective is the solver's own input, `row_builder.get_obj`). No attrs (2026-09-23): the width is `cols.sizes['col']`, a block (the arcs, say) is a mask on `block`, and the names of the codes come from the inputs — `RowInputs.landmans` (what `m` means) and the keys of `RowInputs.agman2lu` (what `am_idx` means, in its order), read off the inputs by `gather`, `get_obj` and `LutoSolver`; readability bought with memory and time (the `block` strings 58 MB at RES5, ~1.4 GB at RES1; a block mask ~76 ms at RES5). The data facts the rows need beside it (`nlms` / `n_ag_lus` / `n_nonag_lus` / `ncells`, `agman2lu`, `savanna_eligible_r`) live on `RowInputs`. Beside the table `get_cols` returns a `ColSupport` (`col_support`): supporting data that maps the sparse column variables onto the dense input arrays, — the input masks: `valid_ag_mrj` (lm, cell, lu), `valid_nonag_rk` (cell, nonag_lu), `valid_ag2ag` / `valid_ag2nonag` / `valid_nonag2ag` {source: mask} (the shape of a source's solved deltas in `post_solve`) — then `region2cell` (`cell_regions`: an `xr.Dataset` on `cell`, no attrs — the `state`, `nrm`, `water_region` and (IBRA mode) `ibra` names as object strings, None where a cell has none — so the GBF families mask a target to `region2cell[layer].values == region` (a region the layer does not know raises) and water and renewables read the layer at their columns' cells; the regional-adoption caps carry their own cell set, read at every column's cell), then the cell × col incidence the layer families multiply with (`cell2col`: built from the table's `cell` field, read by the renewable ceiling and the five GBF families — a layer family's weight rows over cells W become rows over columns as `W @ (cell2col @ diags(c))`) and the position → column grids the join families look up (every ag / non-ag column written at its own position), and nothing else: `ag_mrj2col` (lm, cell, lu) / `nonag_rk2col` (cell, nonag_lu), int32, the table row of the column at each grid entry, −1 = no column (+50 MB at RES5, +1.8 GB at RES1; there is no am grid — the am columns are read off their fields). Every feasible non-ag entry has a column, a disabled land use's fixed at zero, so no value grid is carried for entries without one. The table is the long view, the side its support by position. The module opens with `get_cols`, one function in numbered sections that reads as the module's table of contents, and everything it calls follows below it in that same order: the `get_*` getters (sources, transition bounds, feasibility; the renewable cell sets come from `row_inputs`), then the block builders — every one returns its block's ROWS as a field dict beside the mask it enumerated them from (`ag_space` in the ag inputs' (lm, cell, lu) order, `nonag_space` in (cell, nonag_lu) order — every feasible entry, a disabled land use's fixed at zero — `am_space` in (slot, lm, cell) order, the arc builders per source from their arc masks), — then `table_space(blocks)`, where `blocks` is the flat {block: rows} dict whose declared order IS the table's layout and whose keys are the `block` labels — `ag`, `nonag`, `am` first (a per-cell policy coefficient accounts over them), then the arcs (priced by the objective, no coefficient of their own). `data.AGMAN2LU` ({option: [land-use codes]}, a `Data` property) fixes the ag-management slot order. The row families read the table and its support: every family that is a SUM over cells gathers one float32 coefficient per column (`row_builder.gather`: every column reads its block's input at its own fields, `c[on] = input[m[on], cell[on], j[on]]` for `on` the block's — an option's, a source's — columns; the biodiversity contributions reach the same gather as broadcast views via `bio_contribution`, laid on the support ONCE for all five GBF families) and multiplies the support — `weight_rows(W) @ (cell2col @ diags(c))` for the GBF families, the region of the column's cell (`(region2cell[layer].values == region)[cell]`) for water and renewables, the cap's own cell set read at the column's cell for regional adoption, `cell2col[cells] @ diags(on)` for the renewable ceiling (one unit row per cell); the objective (`get_obj`) and GHG are the same gather over all six blocks (`gather` takes the per-source arc inputs too; the objective negates its arc columns (`block` in ag2ag / ag2nonag / nonag2ag)); the JOIN families look their nodes up on the id grids — the ag-mgt link has one row per am column (X_am − X_ag ≤ 0), its ag column read off `ag_mrj2col[m, cells, j]` (an ag column with no am column gets no row: its lower bound already says X_ag ≥ 0), adoption and demand take their ag / non-ag groups off the grids, node balance finds the row of the node every column and arc lands on / leaves (`ag_mrj2col` / `nonag_rk2col`), −1 = the entry is dropped; source cap groups the arcs by their source's flat position in (lm, lu, cell) order (`np.ravel_multi_index`) and caps each at its source's `base` on the table, found through `ag_mrj2col` (every source has a column: the source map and the ub share one noise floor); the ag-mgt rows (link, adoption, demand) take the am columns off the table by `am_idx` / `j_idx` / `m`. The solver creates ONE `addMVar` over the table (`lb` / `ub` per row, the names from the fields, block by block) and reads the solution back through it. **The transition model is exact** (2026-09-08): every nonzero (lm, lu) fraction of a cell in the base year above the `ROUND_DECIMALS` noise floor is its own source (`data.ag_dvars[base_year]` read as it is; `transitions.get_base_dvar_mj_cell_map`); the former θ fold (`EXACT_REACHABILITY_MIN_FRACTION`, the fold block, the `acct_link_*` rows) is gone — a census showed it touched ≤ 0.02 % of the entries per year at RES5 (`docs/FINDINGS.md`, 20260908).
...
- Adoption limits enforced via existing `const_ag_man_adoption_limit` solver constraints
