...
- **`luto/solvers/`**: Optimization solver interface and input data preparation
  ...
  - `col_builder.py`: the COLUMN side — `get_cols(data, base_year) -> (cols, col_side)` builds the column space from the base-year state alone (transition bounds, feasibility, sources, masks): `cols`, every unknown as one row of the long table, and beside it a `ColSide` — the wide `base` / `ub` grids the flow rows read for entries with no column, the two source maps, the four renewable masks
  ...
  - `row_builder.py`: the row side's ROW GENERATION. The model is one matrix, rows × cols: every family produces ROWS of it the same way — `gather` (one coefficient per accounting column, read by what the column's fields say it is) → `compose_rows` (× the row's weights over cells, the nonzero support as one sparse row) → stack → `contract` (ONE loop over the rows: the `SOLVER_COEFF_MIN` drop, then for the policy families the geomean row rescale and the floor again). The flow and structural rows group and join on node ids (`node_ids`, `ag_runs`, `am_slots`). Each family returns a PART (`make_part`, one fixed row schema); `get_rows(inputs, cols, col_side)` — the module's entry, at the top of the file, its body the model's row order — runs every family and `stack_rows` lays the parts into the ROW TABLE (see below), returned with a `RowSide` (the unscaled production block) beside it. `get_obj(econ, cols, col_side)` is the objective coefficient of every column (million AUD, dropped and floored), attached by the caller as `cols['obj']`. `LutoSolver(cols, rows)` is A x T — the two tables and nothing else; `solve()` returns the raw x; `post_solve.post_solve(x, cols, col_side, rows, row_side, inputs)` turns it into the LUTO format
    - Biodiversity data attributes use `*_pre_1750_area_*` naming (e.g., `GBF3_NVIS_pre_1750_area_vr`, `GBF4_SNES_pre_1750_area_sr`)
    - **No input rescaling** (2026-09-03): the coefficient streams reach the solver raw (float32). Every constraint block is row-rescaled by `row_builder.contract` — per row, scale = geometric mean of max|row| and |RHS| over `RESCALE_FACTOR` (`calc_geomean_scale`), row and RHS divided by it, stage-4 floor on the scaled row — and the factor is kept on the row table (`scale`); `tools._shadow_price_views` hands it to `tools.calc_shadow_price_*` per family (`demand_scales`, `water_scales`, `ghg_scale`, `renewable_scales`, `bio_GBF2_scale`, `bio_*_scales`; So = 1e6: the objective is raw AUD / 1e6). Row scaling is an exact LP transformation; the gate compares models in RESTORED space (rows × their factor).
