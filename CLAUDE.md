...
- **`luto/solvers/`**: Optimization solver interface and input data preparation
  ...
  - `col_builder.py`: the COLUMN side — `get_cols(data, base_year) -> (cols, col_support)` builds the column space from the base-year state alone (transition bounds, feasibility, sources, masks): `cols`, every unknown as one row of the long table, and beside it a `ColSupport` — supporting data that maps the sparse column variables onto the dense input arrays, so the row side never infers an index. The design is a PAIR of handles per subset, one slicing the input and its twin slicing the column table, so a constraint is input[handle] · x[twin]: the mask every block was enumerated from — `valid_ag_mrj`, `valid_nonag_rk`, `valid_am` (one per (option, land use, lm)), `valid_ag2ag` / `valid_ag2nonag` / `valid_nonag2ag` (one per source) — so `input[mask]` is the block's coefficient vector, and the table's `block_range` / `am_range` / `<arc>_range` attrs say where it goes (the twin handles); `region2cell` / `region2col`, the region layers on cell and read at every column's cell (the pair for a region), the region label of every column (one variable per region layer: state / nrm / water_region), so a regional family takes its columns as `region2col[layer] == code`; and, beside them, `cell2col` (cell × col, 1 where a variable sits in the cell: every cell's variables, for the layer families) and the `ag_mrj2col` / `nonag_rk2col` position → column grids (in the inputs' layout, the masks read backwards), −1 = no column, the join families look up
