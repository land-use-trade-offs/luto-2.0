...
- **`luto/solvers/`**: Optimization solver interface and input data preparation
  ...
  - `col_builder.py`: the COLUMN side — `get_cols(data, base_year)` builds the column space from the base-year state alone (transition bounds, feasibility, sources, masks): every unknown as one row of `cols['table']`, plus the three wide id grids the row families index by position
  - `row_inputs.py`: the row side's INPUT DATA, purely downstream of the economics modules — it never sees the column space. `get_rows(data, base_year, target_year)` loads every coefficient stream, target and layer straight from the economics modules and `Data` (no wrapper layer; the setting that turns a family off is read at the call, and a family that is off gets an empty default) and returns them as ONE `RowInputs`; `get_economics(data, base_year, target_year)` does the same for the objective's own streams (cost, revenue, per-arc transition cost) as an `EconomicInputs` — kept separate because it is ~300 MB at RES5 (~7.2 GB at RES1) and is wanted only while the objective block is built, whereas a `RowInputs` lives as long as the solve
  - `row_builder.py`: the row side's ROW GENERATION. The model is one matrix, rows × cols: every family produces ROWS of it the same way — `gather` (one coefficient per scored column, read by what the column's fields say it is) → `compose_rows` (× the row's weights over cells, the nonzero support as one sparse row) → stack → `contract` (ONE loop over the rows: the `SOLVER_COEFF_MIN` drop, then for the policy families the geomean row rescale and the floor again). The flow and structural rows group and join on node ids (`node_ids`, `ag_runs`, `am_slots`). Each family returns a PART (`make_part`, one fixed row schema); `stack_rows` lays the parts into the ROW TABLE (see below). `get_obj_block(econ, cols)` is the objective in the same shape. `LutoSolver(cols, rows, obj_block)` takes the three pieces of the LP
    - Biodiversity data attributes use `*_pre_1750_area_*` naming (e.g., `GBF3_NVIS_pre_1750_area_vr`, `GBF4_SNES_pre_1750_area_sr`)
    - **No input rescaling** (2026-09-03): the coefficient streams reach the solver raw (float32). Every constraint block is row-rescaled by `row_builder.contract` — per row, scale = geometric mean of max|row| and |RHS| over `RESCALE_FACTOR` (`calc_geomean_scale`), row and RHS divided by it, stage-4 floor on the scaled row — and the factor is kept on the solver (`demand_scales`, `water_scales`, `ghg_scale`, `renewable_scales`, `bio_GBF2_scale`, `bio_*_scales`) for the post-solve breakdown and `tools.calc_shadow_price_*` (So = 1e6: the objective is raw AUD / 1e6). Row scaling is an exact LP transformation; the gate compares models in RESTORED space (rows × their factor).
    - `SOLVER_COEFF_MIN` (1e-4): Universal minimum coefficient threshold, applied by `row_builder.contract` to every family's stacked block (and to the objective block; `_setup_objective` scales it `× (1/1e6)` and floors again): (1) an entry is dropped when `|a| < SOLVER_COEFF_MIN` (NaN too); (2) after the row rescale the scaled coefficient is floored again. Every policy coefficient multiplies an ag / ag-mgt / non-ag column directly (the transition model is exact, there is no fold and no accounting layer). Chosen empirically: 1e-3 caused ~3% economic loss; 1e-4 retains meaningful small coefficients while keeping the matrix ratio at 1e8.
...
4. **Solver Input**: `solvers/col_builder.get_cols` (the unknowns), `solvers/row_inputs.get_rows` (the coefficient streams and targets) and `solvers/row_builder.get_obj_block(row_inputs.get_economics(...), cols)` (the objective) prepare the model data; the solver takes `(cols, rows, obj_block)`
   ...
   - **The row table** (`row_builder.FAMILIES` → `stack_rows`): every constraint family is a generator `family(rows, cols) -> part | None` returning its rows as ONE part (`make_part`) over a fixed row schema — int key fields `cell, m, j, k, am_idx, from_m, from_j, from_k, local_r, commodity` (−1 where the family has none) and coded key fields `family, group, region, item, presence, bound, state` (labels) — with per-row `rhs`, `sense`, `name`, `scale` and its block of A in attrs. `stack_rows` lays the parts back to back into `solver.rows_table`, an `xr.Dataset` on `row` = Constr.index: the coded fields as int32 codes into `attrs['vocab']`, `attrs['family_range']` = {family: (start, stop)}, `attrs['keys']` = {family: its key fields}, `attrs['A']` the ONE CSR (rows × n_all), and `active` (a dropped row is flagged off — the table never shrinks). `family_rows` / `rows_where` / `decode` / `keys_of` query it. `LutoSolver.formulate` creates ONE `addMVar` over the column table and ONE `addMConstr` over the row table, names both, keeps the Gurobi handles on the table (`constr`), and publishes the flat handle / scale attributes the shadow-price readers walk from the table's active rows (`_publish`). `remove_constraints_by_name` flags rows inactive and removes them from Gurobi; `restore_constraints_by_name` puts them back; `bio_constraint_index` is a query. `solve()` reads the solution back through the column table and fills `lhs` — `(A @ x) × scale`, every row's raw-unit value, dropped rows included — in ONE mat-vec; the report reads the active rows.
   - **The column space** (`luto/solvers/col_builder.py`): every unknown of the model as ONE row of a long table, `cols['table']` — an `xr.Dataset` on `col` = Var.index, the rows in block order (`ag | nonag | am | ag2ag | ag2nonag | nonag2ag | cell_usage`, `attrs['block_range']` = {block: (start, stop)}, the rows each block owns, `row_builder.block_slice(table, name)` = one block's rows) — with the fields of each column, −1 where a field does not apply: `m, j` (the ag (lm, lu) the column lands on: own for ag, host for am, the TO fields of ag2ag / nonag2ag), `k` (own for nonag, the TO field of ag2nonag), `slot`, `am_idx`, `j_idx` (the (am, lu) slot of an am column, its option as an index into `attrs['options']`, its land use's position within the option), `from_m, from_j, from_k, local_r` (where an arc comes from), `cell` (every column), `lb` / `ub` (float64) and `base` (float32, the node-balance constant). Attrs: `n_terms` (the scored columns: the first three blocks, so a coefficient array indexed by column covers them), `n_dec` (the decision columns; the objective's width), `n_all` (every column; the rows' width), `by_cell_order` / `by_cell_ptr` (the scored rows re-sorted by cell, for the sparse-support gather in `compose_rows`) and `src_ptr` (per arc block, the group bounds of its rows sorted by source, as table rows). The am block reads from the table alone (a column is an ag-mgt column when its `am_idx` >= 0; `attrs['agman2lu']` fixes the (option, lu) slot order, `attrs['savanna_eligible_r']` the cells the read-back zeroes irr savanna outside). Beside the table the space keeps two wide grids for what the flow rows read by grid position for entries with NO column — `cols['ag']` (lm, lu, cell; `base`: a source with no X column still caps its outflow), `cols['nonag']` (nonag_lu, cell; `ub` / `base`, every land use enabled or not: the feasible entries of a disabled land use get a node-balance row) — plus `cols['sources']` and the four `mask_*` arrays. There is no column-id grid: every column is found through the table. The module is ordered data-first: the `get_*` getters (sources, transition bounds, feasibility, masks), then the block builders — every one returns its block's ROWS as a field dict, and `ag_space / nonag_space` return their base / ub grid beside it, while `ag2ag_space / ag2nonag_space / nonag2ag_space` return the `src_ptr` group bounds beside theirs — then `table_space(data, blocks, src_ptr)`, where `blocks` groups them by what they are FOR — `scored` (a per-cell policy coefficient multiplies them), `arcs` (priced by the objective, no coefficient of their own), `slack` (no cost at all) — and the groups' declared order IS the table's layout, so `n_terms` is the scored group's width and `n_dec` everything before the slack group: moving a block between groups moves every count downstream with it, and nothing has to be asserted. `get_cols` is one function in numbered sections: the getters, the blocks, the table. `data.AGMAN2LU` ({option: [land-use codes]}, a `Data` property) fixes the ag-management slot order. The row families read the table: the policy families gather one float32 coefficient per scored column (`row_builder.gather`, by what the column's fields say it is: `k >= 0` non-ag, `am_idx >= 0` ag-mgt, else ag; the biodiversity contributions reach the same gather as broadcast views via `bio_streams`) and multiply it by a weighting row over cells (`compose_rows`); the objective (`get_obj_block`) is the same gather plus the transition costs per source run of the arc blocks; the flow and structural rows group and join on node ids (`row_builder.node_ids`: every column's `to_node`, the (m, j, cell) / (k, cell) it lands on, and every arc's `from_node`) — source cap groups the arcs by `from_node`, node balance joins every X column and every arc on its node, the ag-mgt link joins an am column to its host ag column (`ag_runs` / `am_slots` are the (m, j) and (slot, m) runs of the sorted blocks). The solver creates ONE `addMVar` over the table (`lb` / `ub` per row, the block views as slices, the names from the fields) and reads the solution back through it. **The transition model is exact** (2026-09-08): every nonzero (lm, lu) fraction of a cell in the base year above the `ROUND_DECIMALS` noise floor is its own source (`data.ag_dvars[base_year]` read as it is; `transitions.get_base_dvar_mj_cell_map`); the former θ fold (`EXACT_REACHABILITY_MIN_FRACTION`, the fold block, the `acct_link_*` rows) is gone — a census showed it touched ≤ 0.02 % of the entries per year at RES5 (`docs/FINDINGS.md`, 20260908).
