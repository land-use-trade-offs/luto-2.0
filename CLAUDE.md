...
- **`luto/solvers/`**: Optimization solver interface and input data preparation
  ...
  - `row_table.py`: how the rows are STORED and how they are ASKED — `make_part` (a family's table), `FAMILIES` / `ROW_SCHEMA` / `ROW_FILL` (what `get_rows`' `xr.concat` stacks them with) and `decode` / `rows_where` (its queries; a family's rows are read straight off `rows.attrs['family_range']`), read by `row_bounds.py` and `solvers/tools.py`. Split out of `row_builder.py` on 2026-09-10 so that file holds only `get_rows`, `get_obj` and the families
...
4. **Solver Input**: `solvers/col_builder.get_cols` → `(cols, col_support)` (the unknowns, and what the row side reads beside them); `solvers/row_inputs.get_row_inputs` → `inputs` (the coefficient streams and targets); `solvers/row_builder.get_obj(row_inputs.get_economics(...), cols, col_support)` → `cols['obj']` (the objective coefficient of every column); `solvers/row_builder.get_rows(inputs, cols, col_support)` → `(A, rows)` (the constraints); the solver takes the two tables, `LutoSolver(cols, rows, A)`
