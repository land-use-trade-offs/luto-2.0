...
- **`luto/solvers/`**: Optimization solver interface and input data preparation
  ...
  - `col_builder.py`: the COLUMN side — `get_cols(data, base_year)` builds the column space (every unknown of the step as a labelled `xr.Dataset` per block, from the base-year state alone: exclude matrix, transition bounds, feasibility, sources, arc lists, the θ fold, the accounting columns)
...
- `SOLVER_COEFF_MIN`: Universal minimum coefficient threshold (default: 1e-4). Applied by the builders (`row_builder.compose_rows` for every constraint family, `get_obj_block` + `_setup_objective` for the objective vector) as a two-step contract: coefficient `q = V[cell] · c` dropped when `|q| < SOLVER_COEFF_MIN` (`row_builder.drop`), scaled coefficient floored again after row rescaling. The accounting linking rows (`acct_link_*`, the θ fold) are exempt: never floored, never rescaled. Applies to Economy, Biodiversity-quality, GHG, Water, Renewable, GBF2/3/4/8, Demand/Quantity, and Regional Adoption limits. Chosen empirically: 1e-3 caused ~3% economic loss; 1e-4 retains meaningful small coefficients while keeping the matrix range ratio at 1e8 (well within Gurobi's safe zone). `RESCALE_ZERO_THRESHOLD` was removed — post-rescale zeroing is superseded by this universal filter.
...
4. **Solver Input**: `solvers/col_builder.get_cols` (the unknowns) and `solvers/row_builder.get_rows` (the coefficient streams and targets) prepare the model data; the solver takes `(space, rows)`
   ...
   - **Row blocks** (`row_builder.FAMILIES`): every constraint family is a generator `family(rows, space) -> xr.Dataset | None` returning its rows as ONE keyed block — a MultiIndex coordinate `row` (the family's natural key: `(region, item)`, `(cell,)`, `(am, lu, lm, cell)`, …), per-row `rhs`, `sense`, `name`, `scale`, and `attrs['A']` (the CSR over Var.index, row-scaled where `scale` != 1). `LutoSolver.formulate` creates one `addMVar` per block of the space, then loops the families: one `addMConstr` per block, names, the Gurobi handles attached as `constr`, and the flat handle / scale attributes the shadow-price readers walk are published from the blocks (`_publish`). `solve()` reads the solution back through the space and the blocks.
   - **The column space** (`luto/solvers/col_builder.py`): every unknown of the model as a labelled `xr.Dataset` per block (in Var.index order: the order of the `cols` dict in `get_cols`) — `ag` (lm, lu, cell), `nonag` (nonag_lu, cell), `am` (slot = MultiIndex (am, lu), lm, cell), the flow arc lists `ag2ag / ag2nonag / nonag2ag` (fields `src, from_m, from_j / from_k, local_r, cell, to_m, to_j / to_k`), `fold` (the θ-folded entries only, as fold tables on dims `fold_receiver` / `fold_emitter`: each is its own column; every other accounting entry IS its ag column, aliased in the term stream) and `cell_usage` (the range slacks) — each holding its grids suffixed by their dims — `col_mjr` / `ub_mjr` / `base_mjr`, `col_kr` / `lb_kr` / `ub_kr` / `base_kr`, `col_smr` / `lb_smr`, `col_r` (the column grids are global Var.index, −1 = no variable, `>= 0` is the select) — and `col` per arc for the arc lists. The module is ordered data-first: the `get_*` getters (sources, transition bounds, feasibility, masks), then the block builders `ag_space / nonag_space / am_space / ag2ag_space / ag2nonag_space / nonag2ag_space / fold_space / cell_usage_space` (block-local ids), then `get_cols` — one function in numbered sections that calls the getters, builds the blocks, fixes the layout and shifts every id to its global Var.index, and builds the coefficient support `cols['terms']` (one term per accounting / ag-mgt / non-ag column with its cell), read by every policy family through `row_builder.gather_coeffs` / `compose_rows` and by `get_obj_block`. `data.AGMAN2LU` ({option: [land-use codes]}, a `Data` property) fixes the ag-management slot order. `RowInputs` (row_builder.get_rows) carries the streams and targets, the space carries the unknowns and `terms`; the solver creates one `addMVar` per block from the space, adds one exact linking row per folded accounting column (`row_builder.fold_link_rows`, group `fold_link`, structural) and reads the solution back through the space.
