...
- **`luto/solvers/`**: Optimization solver interface and input data preparation
  ...
  - `row_table.py`: how the rows are STORED and how they are ASKED — `make_part` / `stack_rows` (the table's constructor) and `family_rows` / `decode` / `rows_where` / `keys_of` / `bio_index` (its queries), read by `post_solve.py`, `tools/__init__.py` and `simulation.py`. Split out of `row_builder.py` on 2026-09-10 so that file holds only `get_rows`, `get_obj` and the families
