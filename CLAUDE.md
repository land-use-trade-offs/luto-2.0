...
- **`luto/solvers/`**: Optimization solver interface and input data preparation
  ...
  - `col_builder.py`: the COLUMN side — `get_cols(data, base_year) -> (cols, col_side)` builds the column space from the base-year state alone (transition bounds, feasibility, sources, masks): `cols`, every unknown as one row of the long table, and beside it a `ColSide` — supporting data that maps the sparse column variables onto the dense input arrays, so the row side never infers an index: the mask every block was enumerated from — `valid_ag_mrj`, `valid_nonag_rk`, `valid_am` (one per (option, land use) slot), `valid_ag2ag` / `valid_ag2nonag` / `valid_nonag2ag` (one per source) — so `input[mask]` is the block's coefficient vector, and the table's `block_range` / `am_range` / `<arc>_range` attrs say where it goes; `cell2var`, the sparse cell × col incidence the sum families multiply with; the `col_ag_mjr` / `col_nonag_kr` column-id grids, −1 = no column, the join families look up
