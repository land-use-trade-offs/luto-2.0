...
4. **Solver Input**: `solvers/col_builder.get_cols` (the unknowns) and `solvers/row_builder.get_rows` (the coefficient streams and targets) prepare the model data; the solver takes `(space, rows)`
   ...
   - **The column space** (`luto/solvers/col_builder.py`): every unknown of the model as a labelled `xr.Dataset` per block (`BLOCK_ORDER`) — `ag` (lm, lu, cell), `nonag` (nonag_lu, cell), `am` (slot = MultiIndex (am, lu), lm, cell), the flow arc lists `ag2ag / ag2nonag / nonag2ag` (fields `src, from_m, from_j / from_k, local_r, cell, to_m, to_j / to_k`), `acct` (the accounting view X_acct: `col` aliases the ag column except on θ-folded entries, which are their own columns with fold tables on dims `sliver` / `dominant`) and `cell_usage` (the range slacks) — each holding `exists`, `col` (global Var.index, −1 = no variable) and bounds/base. The module is ordered data-first: the `get_*` getters (bounds and base, sources, feasibility and arcs, masks), then the block builders `ag_space / nonag_space / am_space / acct_space / cell_usage_space` (block-local ids), then `get_cols` — one function in numbered sections that calls the getters, builds the blocks, fixes the layout and shifts every id to its global Var.index, and builds the coefficient support `space['terms']` (one term per accounting / ag-mgt / non-ag column with its cell), read by every policy family through `row_builder.gather_coeffs` / `compose_rows` and by `get_obj_block`. `data.AGMAN2LU` ({option: [land-use codes]}, a `Data` property) fixes the ag-management slot order. `RowInputs` (row_builder.get_rows) carries the streams and targets, the space carries the unknowns and `terms`; the solver creates one `addMVar` per block from the space, adds one exact linking row per folded accounting column (`row_builder.acct_link_rows`, group `acct_link`, structural) and reads the solution back through the space.
