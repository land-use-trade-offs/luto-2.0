...
4. **Solver Input**: `solvers/col_builder.get_cols` (the unknowns) and `solvers/row_builder.get_rows` (the coefficient streams and targets) prepare the model data; the solver takes `(space, rows)`
   ...
   - **The column space** (`luto/solvers/col_builder.py`): every unknown of the model as a labelled `xr.Dataset` per block (in Var.index order: the order of the `cols` dict in `get_cols`) — `ag` (lm, lu, cell), `nonag` (nonag_lu, cell), `am` (slot = MultiIndex (am, lu), lm, cell), the flow arc lists `ag2ag / ag2nonag / nonag2ag` (fields `from_m, from_j / from_k`, `to_m, to_j / to_k`, `local_r, cell`; `attrs['src_ptr']` bounds each source's arc range) and `cell_usage` (the range slacks) — each holding its column-id grid `col` (global Var.index, −1 = no variable, `>= 0` is the select; one key for every block, `col` per arc in the arc tables) and its attribute grids on the same dims — `ub` / `base` (ag), `lb` / `ub` / `base` (nonag), `lb` (am); the dims live on the Dataset, so no key carries a dims suffix. The module is ordered data-first: the `get_*` getters (sources, transition bounds, feasibility, masks), then the block builders `ag_space / nonag_space / am_space / ag2ag_space / ag2nonag_space / nonag2ag_space / cell_usage_space` (block-local ids), then `get_cols` — one function in numbered sections that calls the getters, builds the blocks back to back (each placed at the next free Var.index, so every block holds its actual column ids; `cols['layout']` is just the widths `n_dec` / `n_all`), and builds the coefficient support `cols['terms']` (one term per ag / ag-mgt / non-ag column with its cell), read by every policy family through `row_builder.gather_coeffs` / `compose_rows` and by `get_obj_block`. `data.AGMAN2LU` ({option: [land-use codes]}, a `Data` property) fixes the ag-management slot order. `RowInputs` (row_builder.get_rows) carries the streams and targets, the space carries the unknowns and `terms`; the solver creates one `addMVar` per block from the space and reads the solution back through the space. **The transition model is exact** (2026-09-08): every nonzero (lm, lu) fraction of a cell in the base year above the `ROUND_DECIMALS` noise floor is its own source (`data.ag_dvars[base_year]` read as it is; `transitions.get_base_dvar_mj_cell_map`); the former θ fold (`EXACT_REACHABILITY_MIN_FRACTION`, the fold block, the `acct_link_*` rows) is gone — a census showed it touched ≤ 0.02 % of the entries per year at RES5 (`docs/FINDINGS.md`, 20260908).
