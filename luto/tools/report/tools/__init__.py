...
class LogToFile:
    def __init__(self, file_path, mode="a", buff=0):
    ...
    def __call__(self, func):
        def wrapped_func(*args, **kwargs):
            ...
            # Open the log file
            with open(self.file_path, self.mode, self.buff) as file:
                # Redirect stdout to a temporary stream that writes to both the original stdout and the file
                sys.stdout = self._DualStream(original_stdout, file)
                try:
                    # Execute the function
                    result = func(*args, **kwargs)
                finally:
                    # Ensure that stdout is restored to its original state
                    sys.stdout = original_stdout
            return result
        return wrapped_func

    class _DualStream:
        """A helper class to duplicate the stdout stream to both the console and a file."""
