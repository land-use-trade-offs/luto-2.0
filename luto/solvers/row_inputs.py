...
class RowInputs:
    ...
    ncells: int                                                         # ... and cells
    agman2lu: dict                                                      # {option: [land-use codes]} — the (option, lu) slots of the ag-mgt inputs, in slot order
    savanna_eligible_r: np.ndarray                                      # cells eligible for savanna burning: the read-back zeroes irr savanna columns outside them
    mask_gbf2_solar: np.ndarray                                         # cells solar is kept out of by the GBF2 mask: no am column there, and the renewable row leaves them out
...
# ═══════════════════════════ the renewable cell sets: where solar and wind are kept out ═══════════════════════════
...
def get_row_inputs(data: Data, base_year: int, target_year: int) -> RowInputs:
    ...
    return RowInputs(
        ...
        ncells=data.NCELLS,
        agman2lu=data.AGMAN2LU,
        savanna_eligible_r=np.flatnonzero(data.SAVBURN_ELIGIBLE == 1),
...
class EconomicInputs:
    ...
    flow_cost_ag2nonag: dict                                            # {(from_m, from_j): ndarray[local_r, to_k]} — NaN where the land use has no transition cost
...
def get_economic_mrj(
    ...
    """Operating economics per ag (m, r, j), non-ag (r, k) and ag-management option: revenue − production cost
    under maxprofit, production cost under mincost. The land-use transition cost is not here — it is charged per
    arc (``flow_cost_*``); the two non-flow terms that are, are ``non_ag_t_rk`` and the ag-management adoption cost."""
...
def get_economics(data: Data, base_year: int, target_year: int) -> EconomicInputs:
    ...
    # ag→nonag: the dispatcher gives dict[lu_name → dict[(from_m, from_j) → ndarray(ncells_src)]]; laid out per source
    # as ONE (local_r, to_k) array — the layout of the source's ag2nonag arc mask — NaN where a land use has no cost
    cost_by_src = {}
    for lu_name, per_src in non_ag_transition.get_transition_matrix_ag2nonag(data, base_year, target_year).items():
        ...
        for src, arr in per_src.items():
            cost_by_src.setdefault(src, {})[k] = arr.astype(np.float32)
    flow_cost_ag2nonag = {}
    for src, by_k in cost_by_src.items():
        cost_rk = np.full((next(iter(by_k.values())).size, data.N_NON_AG_LUS), np.nan, dtype=np.float32)
        for k, arr in by_k.items():
            cost_rk[:, k] = arr
        flow_cost_ag2nonag[src] = cost_rk
