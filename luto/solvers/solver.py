...
class InputData:
    ...
    def non_ag_lu2cells(self):
        return {
            k: np.where(self.non_ag_x_rk[:, k])[0]
                for k in range(self.n_non_ag_lus)
        }
...
def formulate_model(input_data: InputData, d_c):
    ...
    X_ag_dry_vars_jr = np.zeros((input_data.n_ag_lus, input_data.ncells), dtype=object)
    X_ag_irr_vars_jr = np.zeros((input_data.n_ag_lus, input_data.ncells), dtype=object)
    X_non_ag_vars_kr = np.zeros((input_data.n_non_ag_lus, input_data.ncells), dtype=object)
    
    for j in range(input_data.n_ag_lus):
        dry_lu_cells = input_data.ag_lu2cells[0, j]
        for r in dry_lu_cells:
            X_ag_dry_vars_jr[j, r] = model.addVar(ub=1, name=f"X_ag_dry_{j}_{r}")

        irr_lu_cells = input_data.ag_lu2cells[1, j]
        for r in irr_lu_cells:
            X_ag_irr_vars_jr[j, r] = model.addVar(ub=1, name=f"X_ag_irr_{j}_{r}")

    for k in range(input_data.n_non_ag_lus):
        lu_cells = input_data.non_ag_lu2cells[k]
        for r in lu_cells:
    ...
    X_ag_man_dry_vars_jr = {
        am: np.zeros((len(am_j_list), input_data.ncells), dtype=object)
        for am, am_j_list in input_data.am2j.items()
    }
    ...
    objective = ( 
                    ...
                    gp.quicksum( ag_obj_mrj[0, input_data.ag_lu2cells[0, j], j] @ X_ag_dry_vars_jr[j, input_data.ag_lu2cells[0, j]]
                    + ag_obj_mrj[1, input_data.ag_lu2cells[1, j], j] @ X_ag_irr_vars_jr[j, input_data.ag_lu2cells[1, j]]
                        for j in range(input_data.n_ag_lus) )
                
                    # Effects on production costs + transition costs for alternative agr. management options
                + gp.quicksum( ag_man_objs[am][0, input_data.ag_lu2cells[0, j], j_idx] @ X_ag_man_dry_vars_jr[am][j_idx, input_data.ag_lu2cells[0, j]]
                    + ag_man_objs[am][1, input_data.ag_lu2cells[1, j], j_idx] @ X_ag_man_irr_vars_jr[am][j_idx, input_data.ag_lu2cells[1, j]]
                        for am, am_j_list in input_data.am2j.items()
                        for j_idx, j in enumerate(am_j_list) )

                    # Production costs + transition costs for all non-agricultural land uses.
                + gp.quicksum( non_ag_obj_rk[:, k][input_data.non_ag_lu2cells[k]] @ X_non_ag_vars_kr[k, input_data.non_ag_lu2cells[k]]
                        for k in range(input_data.n_non_ag_lus) )
    ...
    ag_q_dry_p = [gp.quicksum(input_data.ag_q_mrp[0, :, p] * X_dry_pr[p]) for p in range(input_data.nprs)]
    ag_q_irr_p = [gp.quicksum(input_data.ag_q_mrp[1, :, p] * X_irr_pr[p]) for p in range(input_data.nprs)]

    # Transform quantities to CM/c representation by land management (dry/irr).
    ag_q_dry_c = [
        gp.quicksum(ag_q_dry_p[p] for p in range(input_data.nprs) if input_data.pr2cm_cp[c, p])
        for c in range(ncms)
    ]
    ag_q_irr_c = [
        gp.quicksum(ag_q_irr_p[p] for p in range(input_data.nprs) if input_data.pr2cm_cp[c, p])
        for c in range(ncms)
    ]
    ...
    for am, am_j_list in input_data.am2j.items():
        ...
        for p in range(input_data.nprs):
            for j_idx, j in enumerate(am_j_list):
                ...
                X_ag_man_irr_pr.append(np.zeros(input_data.ncells))
                X_ag_man_dry_pr.append(np.zeros(input_data.ncells))
        ...
        ag_man_q_dry_c = [
            gp.quicksum(ag_man_q_dry_p[p] for p in range(input_data.nprs) if input_data.pr2cm_cp[c, p])
            for c in range(ncms)
        ]
        ag_man_q_irr_c = [
            gp.quicksum(ag_man_q_irr_p[p] for p in range(input_data.nprs) if input_data.pr2cm_cp[c, p])
            for c in range(ncms)
        ]
...
def solve(input_data: InputData, model):
    ...
    non_ag_X_sol_rk = np.zeros((input_data.ncells, input_data.n_non_ag_lus)).astype(np.float32)
    am_X_dry_sol_rj = {am: np.zeros((input_data.ncells, input_data.n_ag_lus)).astype(np.float32) for am in input_data.am2j}
    am_X_irr_sol_rj = {am: np.zeros((input_data.ncells, input_data.n_ag_lus)).astype(np.float32) for am in input_data.am2j}
    ...
    for am, am_j_list in input_data.am2j.items():
        ...
        for j_idx, j in enumerate(am_j_list):
            for r in input_data.ag_lu2cells[0, j]:
                am_X_dry_sol_rj[am][r, j] = model.getVarByName(f"X_ag_man_dry_{am_name}_{j}_{r}").X
            for r in input_data.ag_lu2cells[1, j]:
                am_X_irr_sol_rj[am][r, j] = model.getVarByName(f"X_ag_man_irr_{am_name}_{j}_{r}").X
    ...
    non_ag_X_rk_processed = non_ag_X_sol_rk.argmax(axis=1)[:, np.newaxis] == range(input_data.n_non_ag_lus)
