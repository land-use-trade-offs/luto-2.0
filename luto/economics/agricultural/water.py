...
def get_agricultural_management_water_matrices(data: Data, yr_idx) -> dict[str, np.ndarray]:
    ...
    beef_hir_data = (
        ...
        else np.zeros((data.NLMS, data.NCELLS, len(AG_MANAGEMENTS_TO_LAND_USES['Beef - HIR']))).astype(np.float32)
    )
    sheep_hir_data = (
        get_sheep_hir_effect_w_mrj(data, yr_idx)
        if settings.AG_MANAGEMENTS['Sheep - HIR']
        else np.zeros((data.NLMS, data.NCELLS, len(AG_MANAGEMENTS_TO_LAND_USES['Sheep - HIR']))).astype(np.float32)
    )
