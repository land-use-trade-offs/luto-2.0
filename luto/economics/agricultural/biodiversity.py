...
def get_breq_matrices(data):
    ...
    for j in biodiv_non_penalty_lus:
        b_mrj[:, j, :] = data.BIODIV_SCORE_WEIGHTED

    if settings.BIODIV_LIVESTOCK_IMPACT > 0:
        for j in livestock_lus:
            b_mrj[:, j, :] = data.BIODIV_SCORE_WEIGHTED * (1 - settings.BIODIV_LIVESTOCK_IMPACT)
...
def get_asparagopsis_effect_b_mrj(data) -> np.ndarray:
    ...
    """
    return np.zeros((data.NLMS, data.NCELLS, data.NLUS))
...
def get_biodiversity_limits(data, yr_idx) -> float:
    ...
    biodiv_2010_non_pen_score = (non_penalty_cells_2010 * data.BIODIV_SCORE_WEIGHTED).sum()
    biodiv_2010_pen_score = (1 - settings.BIODIV_LIVESTOCK_IMPACT) * (
        livestock_cells_2010 * data.BIODIV_SCORE_WEIGHTED
    ).sum()
