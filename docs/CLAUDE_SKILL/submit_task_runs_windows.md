...
- `run_all.py` polls for completions every 10 seconds; it is safe to `Ctrl+C` and relaunch. It classifies each `Run_G*` dir first — `finished` runs (have `Run_Archive.zip`) are always skipped automatically, `checkpoint` runs (have `output/*/data_<year>.lz4`) are resumed, and the rest start fresh. No need to pass specific run names just to avoid re-running finished runs.
