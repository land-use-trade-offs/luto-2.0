...
4. **Solver Input**: `col_builder.get_cols` → `(cols, col_support)`; `row_inputs.get_row_inputs` → `inputs`; `row_builder.get_rows(inputs, cols, col_support)` → `(A, rows)`; `row_builder.get_obj(get_economics(...), cols, col_support)` → `obj`; the solver takes them as they are, `LutoSolver(cols, rows, A, obj)`
...
run(data) → solve_timeseries(data, years=sorted(SIM_YEARS))   # default 2020, 2025, …, 2050
    ...
    For each year pair (base→target):
        ...
        ├── obj = row_builder.get_obj(row_inputs.get_economics(...), cols, col_support)   # the objective, built last: a stopped year never loads the economy streams
        ├── LutoSolver(cols, rows, A, obj).formulate()      # A x T, obj · x: ONE addMVar over the column table; ONE addMConstr over the row table's active rows
