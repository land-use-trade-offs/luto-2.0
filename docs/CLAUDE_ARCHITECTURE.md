...
4. **Solver Input**: `col_builder.get_cols` → `(cols, col_support)`; `row_inputs.get_row_inputs` → `inputs`; `row_builder.get_obj(get_economics(...), cols, col_support)` → `cols['obj']`; `row_builder.get_rows(inputs, cols, col_support)` → `(A, rows)`; the solver takes the two tables, `LutoSolver(cols, rows, A)`
...
run(data) → solve_timeseries(data, years=sorted(SIM_YEARS))   # default 2020, 2025, …, 2050
    ...
    For each year pair (base→target):
        ...
        ├── cols['obj'] = row_builder.get_obj(row_inputs.get_economics(...), cols, col_support);  row_builder.get_rows(inputs, cols, col_support) → (A, rows)
        ...
        ├── solve() → x;  post_solve.post_solve(x, cols, col_support, A, rows, inputs) → SolverSolution
