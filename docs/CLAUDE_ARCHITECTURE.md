...
4. **Solver Input**: `col_builder.get_cols` (unknowns) and `row_builder.get_rows` (coefficient streams, targets) prepare the model data; the solver takes `(space, rows)`
   ...
   - The column space (`col_builder.py`: one labelled `xr.Dataset` per block — ag, nonag, am, ag2ag/ag2nonag/nonag2ag, cell_usage — with the column-id grid `col` in every block (global Var.index, −1 = no variable, `>= 0` is the select; `col` per arc in the arc tables) and the attribute grids on the same dims: `ub` / `base` (ag), `lb` / `ub` / `base` (nonag), `lb` (am); no key carries a dims suffix, the dims live on the Dataset) and its coefficient support (`cols['terms']`) are built once in `get_cols` (getters first, then the block builders, then one sectioned `get_cols`) and shared by every constraint family (`row_builder.gather_coeffs` / `compose_rows`) and the objective block. Every policy row scores the ag columns directly: there is no accounting layer.
