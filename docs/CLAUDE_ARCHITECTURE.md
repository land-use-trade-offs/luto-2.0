...
4. **Solver Input**: `col_builder.get_cols` (unknowns) and `row_builder.get_rows` (coefficient streams, targets) prepare the model data; the solver takes `(space, rows)`
   ...
   - The column space (`col_builder.py`): every unknown as one row of the long table `cols['table']` (on `col` = Var.index, the blocks `ag | nonag | am | ag2ag | ag2nonag | nonag2ag | cell_usage` back to back, `attrs['block_ptr']` the block bounds; fields `block, m, j, k, slot, am_idx, j_idx, from_m, from_j, from_k, local_r, cell` with −1 where a field does not apply, `lb` / `ub` / `base` per column; attrs `n_terms` / `n_dec` / `n_all`, `by_cell_*`, `src_ptr` per arc block), beside the wide id grids `cols['ag']` / `cols['nonag']` / `cols['am']` (the column-id grid `col`, global Var.index, −1 = no variable, plus only what is read by grid position: `base` for ag, `ub` / `base` for non-ag, per-slot `j` for ag-mgt — every bound reaches the solver as a table row instead, never in both shapes) that the ag-mgt link / adoption rows and the source-cap base still index. Built once in `get_cols` (getters first, then the block builders — each returns its block's ROWS, the three grid builders their id grid beside it and the arc builders their `src_ptr` — then `table_space`, which lays the rows back to back in the key order of its `parts` argument). Every policy family scores the table's first three blocks (`row_builder.gather_coeffs` / `compose_rows`), the flow rows are group-bys over its arc blocks, and the objective block is built the same way. Every policy row scores the ag columns directly: there is no accounting layer.
