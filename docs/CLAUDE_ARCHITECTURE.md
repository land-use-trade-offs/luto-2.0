...
- **`luto/solvers/`**: Optimization solver interface and input data preparation
  ...
  - `col_builder.py`: the COLUMN side — `get_cols(data, base_year)` builds the column space (every unknown as one row of the long table `cols['table']`, beside the wide `base` / `ub` grids the flow rows read for entries with no column) from the base-year state
  - `row_inputs.py`: the row side's INPUT DATA, purely downstream of the economics modules — it never sees the column space. `get_rows(data, base_year, target_year)` returns every coefficient stream, target and layer as ONE `RowInputs` (no wrapper layer; the setting that turns a family off is read at the call); `get_economics(...)` returns the objective's own streams as an `EconomicInputs`, loaded separately because it is ~300 MB at RES5 and wanted only while the objective block is built
  - `row_builder.py`: the row side's ROW GENERATION — every family builds rows of the one matrix (rows × cols) the same way: `gather` → `compose_rows` → stack → `contract` (drop, then the policy rescale + floor, one loop over the rows); the flow / structural rows group and join on `node_ids`; each family returns a part (`make_part`), `stack_rows` lays them into the row table; `get_obj_block(econ, cols)` is the objective in the same shape (`FAMILIES`, in model order). `LutoSolver(cols, rows, obj_block)` takes the three pieces of the LP
    ...
    - `SOLVER_COEFF_MIN` (1e-4): Universal minimum coefficient threshold, applied by `row_builder.contract` to every family's stacked block (and to the objective block; `_setup_objective` scales it `× (1/1e6)` and floors again): (1) an entry is dropped when `|a| < SOLVER_COEFF_MIN` (NaN too); (2) after the row rescale the scaled coefficient is floored again. Every policy coefficient multiplies an ag / ag-mgt / non-ag column directly. Chosen empirically: 1e-3 caused ~3% economic loss; 1e-4 retains meaningful small coefficients while keeping the matrix ratio at 1e8.
    - No per-family scale factors: every factor is per constraint row, carried on the row table (`scale`) and published on the solver from its active rows.
...
4. **Solver Input**: `col_builder.get_cols` (unknowns), `row_inputs.get_rows` (coefficient streams, targets) and `row_builder.get_obj_block` (the objective) prepare the model data; the solver takes `(cols, rows, obj_block)`
   ...
   - No input rescaling: constraint blocks are row-rescaled in the solver (`row_builder.contract`, factor kept per row); the objective is raw AUD / 1e6
   - The column space (`col_builder.py`): every unknown as one row of the long table `cols['table']` (on `col` = Var.index, the blocks `ag | nonag | am | ag2ag | ag2nonag | nonag2ag | cell_usage` back to back, `attrs['block_range']` = {block: (start, stop)}, the rows each block owns; fields `m, j, k, slot, am_idx, j_idx, from_m, from_j, from_k, local_r, cell` with −1 where a field does not apply, `lb` / `ub` / `base` per column; attrs `n_terms` / `n_dec` / `n_all`, `by_cell_*`, `src_ptr` per arc block), beside two wide grids for what the flow rows read by position for entries with NO column — `cols['ag']` (`base`), `cols['nonag']` (`ub` / `base`) — and `attrs['agman2lu']` / `attrs['savanna_eligible_r']` on the table; there is no column-id grid. Built once in `get_cols` (getters first, then the block builders — each returns its block's ROWS, `ag_space` / `nonag_space` their grid beside it and the arc builders their `src_ptr` — then `table_space`, which lays the rows back to back group by group (`scored | arcs | slack`), so `n_terms` and `n_dec` are group widths rather than positions). Every policy family scores the table's first three blocks (`row_builder.gather` / `compose_rows`), the flow and structural rows group and join on node ids (`node_ids`, `ag_runs`, `am_slots`), and the objective block is built the same way. Every policy row scores the ag columns directly: there is no accounting layer.
...
run(data) → solve_timeseries(data, years=sorted(SIM_YEARS))   # default 2020, 2025, …, 2050
    ...
    For each year pair (base→target):
        ├── col_builder.get_cols(data, base_yr) → cols;  row_inputs.get_rows(data, base_yr, target_yr) → rows;  row_builder.get_obj_block(row_inputs.get_economics(...), cols) → obj
        ├── LutoSolver(cols, rows, obj).formulate()  # ONE addMVar over the column table; ONE addMConstr over the row table
        │   ├── _setup_vars()             # the column table -> gp.MVar, the blocks as slices, names from the fields
        │   ├── _setup_constraints()      # row_builder.FAMILIES -> parts -> stack_rows -> solver.rows_table; handles on the table; _publish()
        │   └── _setup_objective()       # obj_block (5 x n_dec) summed, scaled to million AUD, floored
